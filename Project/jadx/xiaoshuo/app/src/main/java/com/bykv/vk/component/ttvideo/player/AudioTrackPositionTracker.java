...
final class AudioTrackPositionTracker {
    ...
    private final Listener f2059listener;
    ...
    private void maybePollAndCheckTimestamp(long j, long j2) {
        ...
        if (audioTimestampPoller.maybePollTimestamp(j)) {
            ...
            if (Math.abs(timestampSystemTimeUs - j) > 5000000) {
                this.f2059listener.onSystemTimeUsMismatch(timestampPositionFrames, timestampSystemTimeUs, j, j2);
                audioTimestampPoller.rejectTimestamp();
            } else if (Math.abs(framesToDurationUs(timestampPositionFrames) - j2) <= 5000000) {
                audioTimestampPoller.acceptTimestamp();
            } else {
                this.f2059listener.onPositionFramesMismatch(timestampPositionFrames, timestampSystemTimeUs, j, j2);
                audioTimestampPoller.rejectTimestamp();
            }
        }
    }
    ...
    private void maybeUpdateLatency(long j) {
        ...
        try {
            ...
            this.latencyUs = jMax;
            if (jMax > 5000000) {
                this.f2059listener.onInvalidLatency(jMax);
                this.latencyUs = 0L;
            }
    ...
    public boolean mayHandleBuffer(long j) {
        ...
        if (z && !zHasPendingData && playState != 1 && (listener2 = this.f2059listener) != null) {
