...
public class LiveLoggerService implements Handler.Callback {
        ...
        public MyNTPTask(final LiveLoggerService liveLoggerService) {
            ...
            this.mHandler = new Handler(this.mHandlerThread.getLooper()) { // from class: com.bykv.vk.component.ttvideo.log.LiveLoggerService.MyNTPTask.1
                ...
                public void handleMessage(Message message) {
                    String str;
                    if (message == null || (str = liveLoggerService.mHostNTPUrl) == null || str.equals(eq2.g)) {
                        return;
                    }
    ...
    public void ntp() {
        ...
        long time3 = time.getMessage().getReceiveTimeStamp().getTime();
        long time4 = time.getMessage().getTransmitTimeStamp().getTime();
        long jCurrentTimeMillis = System.currentTimeMillis();
        this.mTimeDiff = ((time3 - time2) + (time4 - jCurrentTimeMillis)) / 2;
        Log.d("zdh", "ori:" + time2 + " rec:" + time3 + " trans:" + time4 + " destTs:" + jCurrentTimeMillis + " diff:" + this.mTimeDiff);
    }
    ...
    private void onStreamAbnormalFrame(int i, int i2, int i3) {
        ...
        try {
            jSONObjectCreateCommonLog.put("event_key", "stream_abnormal").put(SpeechConstant.STREAM_TYPE, "video").put("type", i).put("sent_frames", i2).put("received_frames", i3);
            this.mLogUploader.onMonitorLog(jSONObjectCreateCommonLog, MONITOR_LOG_TYPE);
            long j = this.mLogTotalCount;
            if (j > 0) {
                this.mLogTotalCount = j + 1;
    ...
    private void onStreamAbnormalSEI(int i, int i2, int i3) {
        ...
        try {
            jSONObjectCreateCommonLog.put("event_key", "stream_abnormal").put(SpeechConstant.STREAM_TYPE, "video").put("type", i).put("cur_index", i2).put("last_index", i3);
            this.mLogUploader.onMonitorLog(jSONObjectCreateCommonLog, MONITOR_LOG_TYPE);
            long j = this.mLogTotalCount;
            if (j > 0) {
                this.mLogTotalCount = j + 1;
    ...
    private void updateRequestTimeFieldInRtcPlay(JSONObject jSONObject) {
        if (jSONObject.has("event_message")) {
            ...
            String str = TAG;
            MyLog.i(str, "eventMessage:" + strOptString);
    ...
    public JSONObject createCommonLog() {
        ...
        try {
            JSONObject jSONObjectPut = new JSONObject().put("report_version", "5").put("live_sdk_version", "1.4.6.31-lite").put("product_line", PRODUCT_LINE).put("client_timestamp", System.currentTimeMillis());
    ...
    public void onErrorRecovered() {
        ...
        long jCurrentTimeMillis = System.currentTimeMillis() - this.mErrorStartTime;
        Log.i(TAG, "recover from error time " + jCurrentTimeMillis);
    ...
    public void onFirstFrameFail(int i) {
        ...
        if (jSONObjectCreateCommonLog != null) {
            try {
                jSONObjectCreateCommonLog.put("event_key", "first_frame_failed").put("start_time", System.currentTimeMillis()).put(h8.d, i);
                reportQuicOpenResult(jSONObjectCreateCommonLog);
                this.mLogUploader.onMonitorLog(jSONObjectCreateCommonLog, MONITOR_LOG_TYPE);
                long j = this.mLogTotalCount;
                if (j > 0) {
    ...
    public void onLoaderSwitch(int i, String str) {
        ...
        if (jSONObjectCreateCommonLog != null) {
            try {
                ...
                jSONObjectCreateCommonLog.put("event_key", "p2p_switch").put("switch_info", str).put("p2p_vendor", "none").put(h8.d, i);
                this.mLogUploader.onMonitorLog(jSONObjectCreateCommonLog, MONITOR_LOG_TYPE);
                long j = this.mLogTotalCount;
                if (j > 0) {
                    this.mLogTotalCount = j + 1;
    ...
    */
    ...
    public void onRenderStall(long j, int i, boolean z, boolean z2) {
        ...
        if (z) {
            MyLog.i(TAG, "video render stall time " + j);
            ...
            MyLog.i(TAG, "audio render stall time " + j);
    ...
    public void onRetry(int i) {
        ...
        if (jSONObjectCreateCommonLog != null) {
            try {
                jSONObjectCreateCommonLog.put("event_key", "retry").put(h8.d, this.mErrorCode).put("code", i).put("render_fail_type", this.mRenderFailType);
                reportQuicOpenResult(jSONObjectCreateCommonLog);
                this.mLogUploader.onMonitorLog(jSONObjectCreateCommonLog, MONITOR_LOG_TYPE);
                long j = this.mLogTotalCount;
                if (j > 0) {
    ...
    public void onSDKDNSError(LiveError liveError) {
        ...
        if (jSONObjectCreateCommonLog != null) {
            try {
                ...
                jSONObjectPut.put("sdk_params", str).put(SpeechConstant.STREAM_TYPE, this.mLogBundle.mStreamType).put("suggest_format", this.mSuggestFormat).put("suggest_protocol", this.mSuggestProtocol).put("nnsr_enabled", (this.mLiveEngine.getSRState() || this.mLiveEngine.getSharpenState()) ? 1 : 0);
                this.mLogUploader.onMonitorLog(jSONObjectCreateCommonLog, MONITOR_LOG_TYPE);
                long j = this.mLogTotalCount;
                if (j > 0) {
                    this.mLogTotalCount = j + 1;
    ...
    public void onSwitch(String str, String str2, String str3, int i) {
        ...
        try {
            ...
            jSONObjectPut2.put("next_cdn_play_url", str2).put("res_bitrate", this.mResBitrate).put("last_res", this.mLastResolution).put("code", i).put(h8.d, str3);
            this.mLogUploader.onMonitorLog(jSONObjectCreateCommonLog, MONITOR_LOG_TYPE);
            long j = this.mLogTotalCount;
            if (j > 0) {
                this.mLogTotalCount = j + 1;
    ...
    public void onUpdateSeiInfo(String str) {
        ...
        try {
            JSONObject jSONObject = new JSONObject(str);
            if (jSONObject.has(xa3.c)) {
                long j = jSONObject.getLong(xa3.c);
            ...
            if (jSONObject.has("app_data")) {
                ...
                if (jSONObject3.has(str5)) {
                    this.mLogBundle.channelId = jSONObject3.getString(str5);
                }
            } else {
