...
public final class LocalBroadcastManager {
    ...
    private LocalBroadcastManager(Context context) {
        ...
        this.mHandler = new Handler(context.getMainLooper()) { // from class: com.mibi.sdk.common.LocalBroadcastManager.1
    ...
    public boolean sendBroadcast(Intent intent) {
        ...
        synchronized (this.mReceivers) {
            ...
            if (z) {
                MibiLog.v(TAG, "Resolving type " + strResolveTypeIfNeeded + " scheme " + scheme + " of intent " + intent);
            }
            ArrayList<ReceiverRecord> arrayList3 = this.mActions.get(intent.getAction());
            if (arrayList3 != null) {
                if (z) {
                    MibiLog.v(TAG, "Action list: " + arrayList3);
                }
                ...
                while (i2 < arrayList3.size()) {
                    ...
                    if (z) {
                        MibiLog.v(TAG, "Matching against filter " + receiverRecord.filter);
                    }
                    if (receiverRecord.broadcasting) {
                        if (z) {
                            MibiLog.v(TAG, "  Filter's target already added");
                        }
                        i = i2;
                        ...
                        if (iMatch >= 0) {
                            if (z) {
                                MibiLog.v(TAG, "  Filter matched!  match=0x" + Integer.toHexString(iMatch));
                            }
                            ...
                            MibiLog.v(TAG, "  Filter did not match: " + (iMatch != -4 ? iMatch != -3 ? iMatch != -2 ? iMatch != -1 ? "unknown reason" : "type" : "data" : "action" : "category"));
                        }
                    }
