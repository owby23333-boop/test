...
public class aw {
    ...
    public aw(Context context) {
        if (Engine.loadSuccess) {
            try {
                String strPqr = Engine.getInstance(context).pqr(Integer.valueOf(cj.f16632g).intValue(), 1, 0, "");
        ...
        try {
            ...
            for (int i3 = 0; i3 < jSONArray.length(); i3++) {
                ...
                if (!string2.contains("azeroth") && !string2.contains("OkHttp") && !string2.contains("retrofit")) {
                    String strReplace = string2.replaceAll(" ", "").replace("\n", "").replace("\t", "").replace("\u200b", "");
                    ...
                    if (!strSubstring.contains(packageName) && !packageName.contains(strSubstring) && !setA.contains(objSubstring) && !b(i2).contains(objSubstring)) {
                        hashSet.add(strSubstring);
                    }
                }
            }
