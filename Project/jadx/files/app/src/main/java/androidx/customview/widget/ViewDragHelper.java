...
public class ViewDragHelper {
    ...
    private int computeAxisDuration(int i2, int i3, int i4) {
        ...
        return Math.min(iAbs > 0 ? Math.round(Math.abs(fDistanceInfluenceForSnapDuration / iAbs) * 1000.0f) * 4 : (int) (((Math.abs(i2) / i4) + 1.0f) * 256.0f), MAX_SETTLE_DURATION);
    }
    ...
    protected boolean canScroll(@NonNull View view, boolean z2, int i2, int i3, int i4, int i5) {
        int i6;
    ...
    public void captureChildView(@NonNull View view, int i2) {
        ...
        throw new IllegalArgumentException("captureChildView: parameter must be a descendant of the ViewDragHelper's tracked parent view (" + this.mParentView + ad.f20406s);
    }
