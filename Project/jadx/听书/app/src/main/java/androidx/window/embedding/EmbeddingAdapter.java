    ...
    private final class VendorApiLevel1Impl {
        ...
        private final boolean isSplitAttributesSupported(SplitAttributes attrs) {
            ...
            if (0.0d <= value && value <= 1.0d) {
                if (!(attrs.getSplitType().getValue() == 1.0f) && ArraysKt.contains(new SplitAttributes.LayoutDirection[]{SplitAttributes.LayoutDirection.LEFT_TO_RIGHT, SplitAttributes.LayoutDirection.RIGHT_TO_LEFT, SplitAttributes.LayoutDirection.LOCALE}, attrs.getLayoutDirection())) {
                    return true;
                }
            }
            return false;
