...
public final class ConcurrentLinkedListKt {
    ...
    public static final <S extends Segment<S>> Object findSegmentInternal(S s, long j, Function2<? super Long, ? super S, ? extends S> function2) {
        while (true) {
            if (s.id >= j && !s.isRemoved()) {
                return SegmentOrClosed.m2195constructorimpl(s);
            }
            Object nextOrClosed = s.getNextOrClosed();
            if (nextOrClosed == CLOSED) {
                return SegmentOrClosed.m2195constructorimpl(CLOSED);
            }
    ...
    public static final /* synthetic */ <S extends Segment<S>> Object findSegmentAndMoveForward$atomicfu(Object obj, AtomicReferenceFieldUpdater atomicReferenceFieldUpdater, long j, S s, Function2<? super Long, ? super S, ? extends S> function2) {
        ...
        do {
            ...
            if (!SegmentOrClosed.m2200isClosedimpl(objFindSegmentInternal)) {
                Segment segmentM2198getSegmentimpl = SegmentOrClosed.m2198getSegmentimpl(objFindSegmentInternal);
                while (true) {
                    Segment segment = (Segment) atomicReferenceFieldUpdater.get(obj);
                    z = true;
                    if (segment.id >= segmentM2198getSegmentimpl.id) {
                        break;
                    }
                    if (!segmentM2198getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        z = false;
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, obj, segment, segmentM2198getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (segmentM2198getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM2198getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        do {
            ...
            if (!SegmentOrClosed.m2200isClosedimpl(objFindSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceArray, i, segment, segmentM2198getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (segmentM2198getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
