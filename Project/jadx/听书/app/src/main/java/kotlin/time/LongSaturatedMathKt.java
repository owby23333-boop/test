...
public final class LongSaturatedMathKt {
    ...
    public static final long m2090saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long jM2007toLongimpl = Duration.m2007toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m2089checkInfiniteSumDefinedPjuGub4(j, j2, jM2007toLongimpl);
        }
        if ((1 | (jM2007toLongimpl - 1)) == Long.MAX_VALUE) {
            return m2091saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + jM2007toLongimpl;
        return ((j ^ j3) & (jM2007toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* JADX INFO: renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m2089checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m1993isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m2091saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long jM1964divUwyO8pc = Duration.m1964divUwyO8pc(j2, 2);
        long jM2007toLongimpl = Duration.m2007toLongimpl(jM1964divUwyO8pc, durationUnit);
        return ((1 | (jM2007toLongimpl - 1)) > Long.MAX_VALUE ? 1 : ((1 | (jM2007toLongimpl - 1)) == Long.MAX_VALUE ? 0 : -1)) == 0 ? jM2007toLongimpl : m2090saturatingAddNuflL3o(m2090saturatingAddNuflL3o(j, durationUnit, jM1964divUwyO8pc), durationUnit, Duration.m1996minusLRDsOJo(j2, jM1964divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m2063getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m2062getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m2014unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m1997plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m2014unaryMinusUwyO8pc(infinityOfSign(j3));
        }
