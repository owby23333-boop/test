...
public final class ClassName extends TypeName implements Comparable<ClassName> {
    ...
    private ClassName(List<String> list, List<AnnotationSpec> list2) {
        ...
        if (list.get(0).isEmpty()) {
            strJoin = Util.join(FileUtils.FILE_EXTENSION_SEPARATOR, list.subList(1, list.size()));
        } else {
            strJoin = Util.join(FileUtils.FILE_EXTENSION_SEPARATOR, list);
        }
    ...
    public String reflectionName() {
        if (this.names.size() == 2) {
            ...
            return strPackageName + FileUtils.FILE_EXTENSION_SEPARATOR + this.names.get(1);
        }
        StringBuilder sb = new StringBuilder();
        sb.append(topLevelClassName());
        Iterator<String> it = simpleNames().subList(1, simpleNames().size()).iterator();
        while (it.hasNext()) {
            sb.append(Typography.dollar).append(it.next());
        }
        return sb.toString();
    }
