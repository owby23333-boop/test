...
public final class ClassPath {
        ...
        if (value != null) {
            for (String str : CLASS_PATH_ATTRIBUTE_SEPARATOR.split(value)) {
                try {
                    ...
                    if (classPathEntry.getProtocol().equals(FileUtils.URI_TYPE_FILE)) {
    ...
    static ImmutableMap<File, ClassLoader> getClassPathEntries(ClassLoader classLoader) {
        ...
        while (it.hasNext()) {
            URL next = it.next();
            if (next.getProtocol().equals(FileUtils.URI_TYPE_FILE)) {
        ...
        for (String str : Splitter.on(StandardSystemProperty.PATH_SEPARATOR.value()).split(StandardSystemProperty.JAVA_CLASS_PATH.value())) {
            try {
                try {
                    ...
                    builder.add(new URL(FileUtils.URI_TYPE_FILE, (String) null, new File(str).getAbsolutePath()));
                }
    ...
    static File toFile(URL url) {
        Preconditions.checkArgument(url.getProtocol().equals(FileUtils.URI_TYPE_FILE));
        try {
