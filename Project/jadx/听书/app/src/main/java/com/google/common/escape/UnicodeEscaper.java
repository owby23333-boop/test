    ...
    protected static int codePointAt(CharSequence charSequence, int i, int i2) {
        ...
        if (i < i2) {
            ...
            if (cCharAt > 56319) {
                ...
                throw new IllegalArgumentException(new StringBuilder(String.valueOf(strValueOf).length() + 88).append("Unexpected low surrogate character '").append(cCharAt).append("' with value ").append((int) cCharAt).append(" at index ").append(i3 - 1).append(" in '").append(strValueOf).append("'").toString());
            }
            if (i3 == i2) {
                return -cCharAt;
            }
